...
export function Leaderboard({
  ...
  if (members.length === 0) {
    return <p className="text-sm text-foreground/50">No family members yet.</p>;
  }

  return (
    <div className="flex flex-col gap-2">
      {members.map((member, index) => (
        <div key={member.user_id} className="glass-card p-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-medium">
                ...
                {member.user_id === currentUserId && (
                  <span className="ml-1 text-xs text-foreground/50">(you)</span>
                )}
              </div>
              {member.topCategories.length > 0 && (
                <div className="mt-0.5 flex gap-2 text-xs text-foreground/50">
